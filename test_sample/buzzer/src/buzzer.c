...
static const int g_scoreDurations[] = {
...
int main(int argc, char **argv)
{
    ...
    // 演奏音乐
    // 使用循环遍历曲谱音符数组
    for (int i = 0; i < sizeof(g_scoreNotes)/sizeof(g_scoreNotes[0]); i++) {
        // 获取音符，也就是分频倍数数组元素的下标
        int tune = g_scoreNotes[i];
        // 获取分频倍数
        int freqDivisor = g_tuneFreqs[tune];
        // 获取音符时间
        // 适当拉长时间，四分之一拍用时125ms，每小节2s
        int tuneInterval = g_scoreDurations[i] * (125*1000);
        // 日志输出
        // 开始输出PWM信号，占空比50%
        set_pwm_period(pwmChannel, freqDivisor);
        set_pwm_dutyCycle(pwmChannel,freqDivisor/2);
		set_pwm_polarity(pwmChannel, PWM_POLARITY_NORMAL);
